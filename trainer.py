...
class Trainer():
    def __init__(self, args, model, loss, loader, ckpt):
        ...
        self.attackset = loader.attackset
        self.attack_loader = loader.attack_loader
    ...
    def attack(self):
        ...
        for batch, (inputs, labels) in enumerate(self.attack_loader):
            inputs = inputs.to(self.device)
            labels = labels.to(self.device)
            ...
            if(batch == 3):
                print(outputs.shape)
        print(x.shape)
        # x = self.extract_feature(self.attack_loader)
        # loss = x[4][0]
        # for i in range(50):
        #     loss+=x[4][i]
        # #loss = x[4][45]
        ...
        # print(x[4].shape)
        # print(x[5].shape)
        # for i,img in enumerate(self.attack_loader):
        #     f = self.extract_feature(img).numpy()


        # print(self.attack_loader.numpy().shape())
        # af = self.extract_feature(self.attack_loader).numpy()
    ...
    def test(self):
        ...
        if self.args.re_rank:
            ...
            dist = cdist(qf, gf)
        r = cmc(dist, self.attackset.ids, self.testset.ids, self.attackset.cameras, self.testset.cameras,
                separate_camera_set=False,
                single_gallery_shot=False,
                first_match_break=True)
        m_ap = mean_ap(dist, self.attackset.ids, self.testset.ids, self.attackset.cameras, self.testset.cameras)
